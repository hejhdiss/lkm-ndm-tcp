...
#define NDM_TCP_VERSION "4.0(delay-enhanced)"
...
static void ndm_forward_pass(struct ndm_tcp *ca, u32 rtt_us, u32 *cwnd_delta)
{
	...
	/* --- INPUT 6: RTT Variation (Jitter) --- */
	/* We calculate the absolute difference between current RTT and the last recorded RTT */
	u32 q_delay = (rtt_us > ca->min_rtt_us) ? (rtt_us - ca->min_rtt_us) : 0;
	inputs[6] = (s32)min_t(u64, (q_delay * 1000ULL) / max(ca->min_rtt_us, 1U), 1000);

	/* --- Proposed Input 8: RTT Gradient --- */
	/* (current_RTT - previous_RTT). Scaled to ±1000 */
